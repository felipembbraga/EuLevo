...
DEFAULT_FCM_DEVICE_MODEL = 'firebasecm.Device'
...
class FirebaseMessage(object):
    def __init__(self):
        ...
        """
        try:
            self.service = FCMNotification(api_key=settings.FCM_APIKEY)
    ...
    def send(self, registration_id, *args, **kwargs):
        return self.service.notify_single_device(registration_id=registration_id, *args, **kwargs)
    def send_multiple(self, registration_ids, *args, **kwargs):
        valid_registration_ids = self.service.clean_registration_ids(registration_ids)
        return self.service.notify_multiple_devices(registration_ids=valid_registration_ids, *args, **kwargs)
